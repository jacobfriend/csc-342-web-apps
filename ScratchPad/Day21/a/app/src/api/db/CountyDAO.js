...
module.exports = {
  getCounties: () => {
    return db.query('SELECT * FROM county')
      .then(rows => {
  ...
  getCountyById: (countyId) => {
    return db.query('SELECT * FROM county WHERE cty_id = ?', [countyId])
      .then(rows => {
        if(rows.length === 1) {
  ...
  createCounty: (countyName) => {
    return db.query('INSERT INTO county (cty_name) VALUES (?)', [countyName])
      .then(result => {
